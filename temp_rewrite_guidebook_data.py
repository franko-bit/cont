...
replacements = {
    'daily-routines.yaml': '''  guidebook_data:
...
base = Path(r'c:\xampp\htdocs\language-platform\content\EN-TO-RW\level2')
for file_name, replacement in replacements.items():
    path = base / file_name
    text = path.read_text(encoding='utf-8')
    start = text.find('  guidebook_data:')
    end = text.find('\n  guidebook: |')
    if start == -1 or end == -1:
        raise RuntimeError(f'Markers not found in {file_name}')
