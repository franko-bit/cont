...
function generateAudioFile($text, $audioDir) {
    ...
    // BETTER VERSION (RECOMMENDED FOR SWAHILI)
    $endpoint = 'https://api.elevenlabs.io/v1/text-to-speech/' . rawurlencode($voiceId);

    $payload = [
        'text' => $text,
        'model_id' => 'eleven_multilingual_v2', // safest for Swahili
        'voice_settings' => [
            'stability' => 0.6,
...
// MODIFIED: Only extract tts_text
function collectTextCandidates($value, &$texts) {
    if (is_array($value)) {
        foreach ($value as $k => $v) {
            // Only look for 'tts_text' key
            if (is_string($k) && $k === 'tts_text') {
                if (is_string($v) && trim($v) !== '') {
                    $texts[] = trim($v);
                }
            }
            // Recursively search nested arrays for tts_text
            if (is_array($v)) {
    ...
    // If value is a string but not an array, ignore (we only want tts_text)
    return;
}
...
function getSwahiliContentRoots($contentRoot) {
    ...
    foreach (scandir($contentRoot) as $entry) {
        ...
        if (strpos($entry, 'SW') !== false || strpos($entry, 'sw') !== false) {
            $roots[] = $path;
        }
    }
...
if (empty($swRootList)) {
    jsonError('No Swahili content folders were found.');
}
